...
//    func fetchCafeMenu(cafeId: String, completion: @escaping (Result<Menu?, Error>) -> Void) {
//        let cafesRef = databaseRef.child("cafes").child(cafeId).child("menu")
//        cafesRef.observeSingleEvent(of: .value) { snapshot, error  in
//            guard let cafesData = snapshot.value as? [String: [String: Any]] else {
//                completion(.success(Menu())) // Return an empty dictionary if no cafes found
//                return
//            }
//
...
//    private func createMenuObject(from data: [String: Any], cafeId: String) -> Menu? {
//        guard
...
//    }
    ...
    func fetchCoffeeData(coffeeId: String, completion: @escaping (Result<Coffee?, Error>) -> Void) {
        let cafeRef = databaseRef.child("coffee").child(coffeeId)
        cafeRef.observeSingleEvent(of: .value) { snapshot, str in
            if let coffeeData = snapshot.value as? [String: Any] {
                // Extract cafe data from snapshot
                let coffee = self.createCoffeeObject(from: coffeeData, coffeeId: coffeeId)
    ...
    private func createCoffeeObject(from data: [String: Any], coffeeId: String) -> Coffee? {
        ...
        let coffee = Coffee(coffeeId: coffeeId, name: name, annotation: annotation, imageURL: imageURL)
        return coffee
    }
    ...
    private func createCafeObject(from data: [String: Any], cafeId: String) -> Cafe? {
        guard
            ...
            let annotation = data["annotation"] as? String,
            let imagesURL = data["images"] as? [String: String],
            let menuDict = data["menu"] as? [String: [String: Any]]
        else {
            print(data)
            return nil
        }
        let coordinates = CLLocationCoordinate2D(latitude: latitude, longitude: longitude)
        let cafe = Cafe(cafeId: cafeId, name: name, email: email, coordinates: coordinates, annotation: annotation, imagesURL: imagesURL, menu: menuDict)
        print("Created \(name) with photos \(imagesURL) ")
        return cafe
    }
